...
public class BC001_Payload : IPayload
{
    ...
    public BC001_Payload(string narration, bool isCustomOn = false,bool isPopFromZero = true)
    {
        IsCustom = isCustomOn;
        Narration = narration;
...
public class BC001_GameManager : Ex_BaseGameManager, IOnClicked
{
    ...
    private void OnRemovalSession()
    {
        ...
        Managers.Sound.Play(SoundManager.Sound.Effect, "BC001/OnAllLeavesRemoved");

        DOVirtual.DelayedCall(1.5f, () =>
        {
            
            Managers.Sound.Play(SoundManager.Sound.Narration, "BC001/LetsClean");
            Messenger.Default.Publish(new BC001_Payload("색깔에 맞춰 낙엽을 터치해주세요!", true));
        });
    ...
    protected override void OnGameStartStartButtonClicked()
    {
        ...
        Messenger.Default.Publish(new BC001_Payload("낙엽을 밟으며 날려봐요!", true));
        Managers.Sound.Play(SoundManager.Sound.Narration, "BC001/PlayWithLeaves");
    }
    ...
    private void ChangeCurrentLeaveColorToRemove(LeaveColor leaveColor)
    {
        ...
        DOVirtual.DelayedCall(1.0f, () =>
        {
            ...
            {
                case LeaveColor.Red:
                    Messenger.Default.Publish(new BC001_Payload("빨강색 낙엽을 터치해 없애주세요!", true));
                    Managers.Sound.Play(SoundManager.Sound.Narration, "BC001/TouchRed");
                    break;
                case LeaveColor.Brown:
                    Messenger.Default.Publish(new BC001_Payload("갈색 낙엽을 터치해 없애주세요!", true));
                    Managers.Sound.Play(SoundManager.Sound.Narration, "BC001/TouchBrown");
                    break;
                case LeaveColor.Yellow:
                    Messenger.Default.Publish(new BC001_Payload("노란색 낙엽을 터치해 없애주세요!", true));
                    Managers.Sound.Play(SoundManager.Sound.Narration, "BC001/TouchYellow");
                    break;
            }
    ...
    protected override void Init()
    {
        ...
        psResourcePath = "Runtime/BC001/Fx_Click";
        base.Init();
    ...
    public override void OnRaySynced()
    {
        ...
        foreach (var _hit in GameManager_Hits)
        {
            ...
            else
            {
                ...
                Managers.Sound.Play(SoundManager.Sound.Effect, "BC001/LeaveClick" + randomChar,0.1F);
    ...
    {
        Managers.Sound.Play(SoundManager.Sound.Effect, "BC001/OnAllLeavesRemoved");
        Managers.Sound.Play(SoundManager.Sound.Narration,"BC001/OnFinishCleaningLeaves");
        DOVirtual.DelayedCall(0.5f,()=>
        {
            Messenger.Default.Publish(new BC001_Payload("잘 했어! 이제 다른 낙엽도 청소하자!", true));
        });
    }
    private void OnRaySyncedOnRemovalSession(Vector3 pos)
    {
        if (currentLeaveColorToRemove == LeaveColor.Red)
        {
            ...
            if (currentRemovedCount >= LEAVES_COUNT_TO_REMOVE)
            {
                ...
                for (int i = 0; i < numParticlesAliveA; i++)
                {
                  ...
                  //      if(currentRemovedCount!=LEAVES_COUNT_TO_REMOVE)Messenger.Default.Publish(new BC001_Payload($"{ Mathf.Clamp(LEAVES_COUNT_TO_REMOVE - currentRemovedCount, 0, LEAVES_COUNT_TO_REMOVE)}개", true,false));
        ...
        else if (currentLeaveColorToRemove == LeaveColor.Yellow)
        {
            ...
            if (currentRemovedCount >= LEAVES_COUNT_TO_REMOVE)
            {
                ...
                {
                 //   RemoveAllLeaves();
                    Managers.Sound.Play(SoundManager.Sound.Narration,"BC001/OnAllFinished");
                    Messenger.Default.Publish(new BC001_Payload("낙엽을 전부 치웠어!", true,true));
                });
    ...
    private void RemoveParticles(Vector3 position, ParticleSystem particleSystem)
    {
        ...
        else
        {
            if (!_isRemoving)
            {
                ...
                Managers.Sound.Play(SoundManager.Sound.Effect, "BC001/Click" + randomChar);
