...
public class AnimalShaderController : MonoBehaviour
{
    ...
    void Update()
    {
        if (AnimalTripBaseGameManager.isRoundStarted)
        {
...
// Load the material from the Resources folder as before
        var tempMat = Resources.Load<Material>("SortedByScene/가을소풍/" + _animalData.englishName);
...
// Get the SkinnedMeshRenderer
    ...
    IEnumerator TurnOffOutLineWithLerpCoroutine()
    {
        ...
        while (true)
        {
            ...
            if (AnimalTripBaseGameManager.isCorrected && _animalData.englishName == AnimalTripBaseGameManager.answer)
            {
