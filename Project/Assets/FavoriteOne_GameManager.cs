...
public class FavoriteOne_GameManager : IGameManager
{
    ...
    private IEnumerator OnSelectCo(Transform button)
    {
        ...
        if (_currentStage == (int)Stage.Animal)
        {
            ...
            {
                _animators[animal.GetInstanceID()].SetTrigger(ON_SELECTED);
        ...
        if (_currentStage == (int)Stage.Fruit)
        {
            ...
            foreach (var ball in _colorBalls)
            {
                ...
                ball.DOScale(_defaultColorBallSize, 0.5f)
    ...
    private void Reinit()
    {
        ...
        DOVirtual.Float(0, 0, restartDelay, _ => { }).OnComplete(() =>
        {
            ...
            {
                splat.DOScale(Vector3.zero, 0.5f).SetEase(Ease.InOutSine);
            }
