...
public class Crab_UIManager : UI_PopUp
{
    ...
    public override bool Init()
    {
        ...
        Crab_SpeechBubble = GetObject((int)SpeechBubble.Crab_SpeechBubble);
    ...
    {
        DOVirtual.Float(0,1,12f, _ => { })
            .OnComplete(() =>
            {
                Crab_SpeechBubble.SetActive(true);
                _rectSpeechBubble.DOScale(_defaultScale, 1.5f).SetEase(Ease.OutBounce);
            
                
                _dialogues[_currentUiIndex].gameObject.SetActive(true);
                _dialogues[_currentUiIndex].DOScale(_defaultScale, 1.5f).SetEase(Ease.OutBounce);

                _currentUiIndex = (int)CrabDialogue.Crab_Dialogue2;
            });
    }
    ...
    private void OnRaySyncForUI()
    {
        ...
        if (_currentUiIndex < (int)CrabDialogue.Max)
        {
            ...
            _dialogues[_currentUiIndex - 1].DOScale(Vector3.zero, 1.5f).SetEase(Ease.OutBounce).OnComplete(() =>
            {
                ...
                DOVirtual.Float(0, 0, 1f, _ => { })
                    .OnComplete(() =>
                    {
                        ...
                        _dialogues[_currentUiIndex].gameObject.SetActive(true);
                        _dialogues[_currentUiIndex].DOScale(_defaultScale, 1.5f).SetEase(Ease.OutBounce).OnComplete(() =>
                        {
    ...
    {
        _dialogues[(int)CrabDialogue.Crab_Dialogue5].DOScale(Vector3.zero, 1.5f).SetEase(Ease.OutBounce)
            .OnComplete(() =>
            {
