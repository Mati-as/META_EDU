...
public class ShootOut_AnimalController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        for (var i = 0; i < lookAtParent.childCount; i++) _lookAts[i] = lookAtParent.GetChild(i).position;

        _animals = new Transform[(int)Animal.Max];

        for (var i = 0; i < (int)Animal.Max; i++) _animals[i] = transform.GetChild(i);
    ...
    private void DoPopUpAnimation(Transform animal)
    {
        ...
        animal.DOScale(_currentDefaultSize, 1.2f).SetEase(Ease.InOutSine)
            .OnStart(() => { animal.DOLookAt(_lookAts[(int)LookAt.OnPop], 0.12f); })
            .OnComplete(() =>
            {
