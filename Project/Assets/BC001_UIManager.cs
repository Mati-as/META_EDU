...
public class BC001_UIManager : Base_UIManager
{
private BC001_GameManager _gm;

    private void Awake()
    {
        // 메시지 구독
        Messenger.Default.Subscribe<BC001_Payload>(OnGetMessageEventFromGm);
        if (_gm == null) _gm = GameObject.FindWithTag("GameManager").GetComponent<BC001_GameManager>();
    ...
    {
        // 구독 해제
        Messenger.Default.Unsubscribe<BC001_Payload>(OnGetMessageEventFromGm);
    }

    private string _foodNarPath = "Audio/SortedByScene/EA009/Food/";

    private void OnGetMessageEventFromGm(BC001_Payload payload)
    {
        if (payload.IsCustom)
        {
